...
rgb Green      = [0, 1, 0]
rgb Blue       = [0, 0, 1]
rgb (Invert c) = map (1 -) (rgb c)  -- Subtract each channel from 1
rgb (Mix c1 c2) = zipWith avg (rgb c1) (rgb c2)  -- Average each channel
...
fromBin :: Bin -> Int
fromBin = fromBinHelper 0
  where
    fromBinHelper :: Int -> Bin -> Int
    fromBinHelper _ End     = 0
    fromBinHelper exp (O b) = fromBinHelper (exp + 1) b
    fromBinHelper exp (I b) = 2 ^ exp + fromBinHelper (exp + 1) b


toBin :: Int -> Bin
toBin 0 = O End  -- Special case for 0
toBin n = toBinHelper n
  where
    toBinHelper :: Int -> Bin
    toBinHelper 0 = End
    toBinHelper x
      | x `mod` 2 == 0 = O (toBinHelper (x `div` 2))
