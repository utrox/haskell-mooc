...
buildList start count end
    | count <= 0 = [end]                -- Base case: if count is 0, return [end]
    | otherwise  = start : buildList start (count - 1) end  -- Recursive case
...
sums i = helper 1 0
  where
    helper n acc
      | n > i = []
      | otherwise = (acc + n) : helper (n + 1) (acc + n)
...
sumsOf xs = helper xs 0
  where
    helper [] _ = []
    helper (y:ys) acc = (acc + y) : helper ys (acc + y)
