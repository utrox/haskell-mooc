...
middle x y z = head . tail . sort $ [x, y, z]
...
longest :: (Ord a) => [[a]] -> [a]
longest = foldl1 (\acc x -> if length x > length acc || (length x == length acc && head x < head acc) then x else acc)
...
transfer from to amount bank
    ...
    | otherwise =
        let fromAmount = Map.findWithDefault 0 from bank
        in if fromAmount < amount then bank
           else Map.adjust (+ amount) to (Map.adjust (subtract amount) from bank)
