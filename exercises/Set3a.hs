...
[] +|+ [] = []
-- If one of the lists is empty, and the other is not, then the 
-- first element of the non-empty list is the only output.
(x:_) +|+ [] = [x]
[] +|+ (y:_) = [y]
-- If both lists are not empty, first element of both lists is the output.
(x:_) +|+ (y:_) = [x, y]
...
interpreter commands = interpret commands 0 0
  where
    interpret [] _ _ = []
    interpret (cmd:cmds) x y =
      case cmd of
        "up"     -> interpret cmds x (y + 1)
        "down"   -> interpret cmds x (y - 1)
        "left"   -> interpret cmds (x - 1) y
        "right"  -> interpret cmds (x + 1) y
        "printX" -> show x : interpret cmds x y
        "printY" -> show y : interpret cmds x y
        _        -> interpret cmds x y