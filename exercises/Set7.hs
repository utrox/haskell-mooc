...
add x (Set xs) 
  | x `elem` xs = Set xs  -- If already in the set, return the same set
  | otherwise   = Set (insertInOrder x xs)  -- Insert x in the correct order

insertInOrder :: (Ord a) => a -> [a] -> [a]
insertInOrder x [] = [x]
insertInOrder x ys@(y:ys')
  | x < y     = x : ys
...
step Finished _ = Finished -- Once it's done, nothing affects it anymore

step _ _ = Error -- Anything invalid defaults to Error
...
average (x :| xs) = sumAll / fromIntegral count
  where
    sumAll = x + sum xs
    count = 1 + length xs
...
reverseNonEmpty (x :| xs) = lastElem :| initElems
  where
    lastElem = last (x : xs)
    initElems = reverse (init (x : xs))
...
instance (Ord a) => Semigroup (Set a) where
  (Set xs) <> (Set ys) = Set (orderedUnion xs ys)

orderedUnion :: (Ord a) => [a] -> [a] -> [a]
orderedUnion [] ys = ys
orderedUnion xs [] = xs
orderedUnion (x:xs) (y:ys)
  | x < y     = x : orderedUnion xs (y:ys)
  | x == y    = x : orderedUnion xs ys
  | otherwise = y : orderedUnion (x:xs) ys
...
passwordAllowed password requirement = case requirement of
    ...
    ContainsSome chars -> any (`elem` password) chars
    DoesNotContain chars -> all (`notElem` password) chars
...
operation op left right = Op op left right

evaluate :: Arithmetic -> Integer
evaluate (Lit n) = n
evaluate (Op "+" left right) = evaluate left + evaluate right
evaluate (Op "*" left right) = evaluate left * evaluate right

render :: Arithmetic -> String
render (Lit n) = show n
render (Op op left right) = "(" ++ render left ++ op ++ render right ++ ")"